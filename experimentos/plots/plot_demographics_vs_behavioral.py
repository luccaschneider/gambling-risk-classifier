...
df = pd.read_csv(METRICAS / "comparison_demographics_vs_behavioral_only.csv")

CLASS_ORDER = ["Baixo risco", "Medio risco", "Alto risco"]
METRICS = ["AUC", "F1", "Recall"]
FEATURE_COLORS = {"Com demograficas": "#2a78d6", "Apenas comportamentais": "#eb6834"}  # slot1 blue / slot2 orange
...
def comparison_chart(model_name, filename):
    ...
    for ax, metric in zip(axes, METRICS):
        for i, feat in enumerate(["Com demograficas", "Apenas comportamentais"]):
            vals = [
    ...
    fig.suptitle(
        f"{model_name}: com demograficas x apenas variaveis comportamentais",
...
comparison_chart("Random Forest", FIGURAS / "grafico_demograficas_vs_comportamental_rf.png")
comparison_chart("XGBoost", FIGURAS / "grafico_demograficas_vs_comportamental_xgb.png")
