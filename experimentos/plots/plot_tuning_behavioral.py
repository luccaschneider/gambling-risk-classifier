...
df = pd.read_csv(METRICAS / "tuning_behavioral_before_after_by_class.csv")

CLASS_ORDER = ["Baixo risco", "Medio risco", "Alto risco"]
METRICS = ["AUC", "F1", "Recall"]
PHASE_COLORS = {"Antes": "#2a78d6", "Depois": "#eb6834"}
...
def tuning_chart(model_name, filename):
    ...
    fig.suptitle(
        f"{model_name} (12 var. comportamentais): antes x depois do tuning (recall macro)",
...
tuning_chart("Random Forest", FIGURAS / "grafico_tuning_behavioral_rf.png")
tuning_chart("XGBoost", FIGURAS / "grafico_tuning_behavioral_xgb.png")
